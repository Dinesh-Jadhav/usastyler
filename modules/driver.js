...
module.exports = {
    ...
    setState: async function (driverId, state) {
        ...
        return pool.query("UPDATE driver SET status = ? WHERE id = ?", [state, driverId]);
    },
    ...
    getDriverInfo: async function (driverId) {
        let result = await pool.query("SELECT driver.id as id, driver.car_image AS car_image,driver.driver_image AS driver_image,driver.first_name AS first_name,driver.last_name AS last_name, driver.rating AS rating,car.title AS car_name, driver.gender AS gender FROM driver LEFT JOIN car ON car.id = fk_car_id where driver.id = ?", [driverId]);
        return result[0][0];
    },
